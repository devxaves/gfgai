...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 bg-black/30 backdrop-blur-sm z-30" onClick={() => setSidebarOpen(false)} />
      )}
      ...
      `}>
          ...
          {links.map((link) => {
            ...
            return (
              <Link key={link.name} href={link.href}
                ...
                <Icon className={`h-[18px] w-[18px] ${isActive ? 'text-indigo-600 dark:text-indigo-400' : ''}`} />
                <span>{link.name}</span>
              </Link>
            );
          })}
        ...
        {conversationHistory.length > 0 && (
          <div className="mt-3 px-2 py-3 border-t border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={clearConversationHistory} className="text-gray-400 hover:text-red-500 transition-colors" title="Clear history">
                <Trash2 className="w-3 h-3" />
              </button>
            </div>
            <div className="space-y-0.5">
              {conversationHistory.slice(-3).reverse().map((entry) => (
                <div key={entry.id} className="text-xs text-gray-500 dark:text-gray-400 truncate py-1 px-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900 cursor-default" title={entry.query}>
                  {entry.query}
                </div>
              ))}
            </div>
        ...
        <div className="mt-auto px-2 py-3 border-t border-gray-100 dark:border-gray-800 space-y-1">
